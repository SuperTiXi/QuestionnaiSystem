...
window.operateEvents = {
    ...
    'click #Tbtn_save': function (e, value, row, index) {
        id = row.no;
        // //console.log(id);
        layer.confirm('您确认要保存此条人员信息吗？', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            var data = {
           ...
           commonAjaxPost(false,"/group/insert",data,function (res) {
               if(res.code == '666'){
                   layer.msg(res.message,{icon: 1});
               }
               else {
                   layer.msg("用户名重复",{icon: 1});
               }
           })
        });
    },
    'click #Tbtn_choose':function (e, value, row, index){
        id = row.id;
        // //console.log(id);
        layer.confirm('您确认要选择该组吗？', {
            btn: ['确定', '取消'] //按钮
        }, function () {
            var data = {
                "groupId":row.id,
...
function TableInit() {
    ...
    oTableInit.Init = function () {
        ...
        $('#groupInfoTable').bootstrapTable({
            url:httpRequestUrl+"/user/list?userId="+userId,
            method:"GET",
            striped: true,                      //是否显示行间隔色
            pagination: true,                   //是否显示分页（*）
            sortOrder: "asc",                   //排序方式
...
function addFunctionAlty(value, row, index) {
    return [
        ...
        '<button id="Tbtn_save" style=" background-color: #f9f9f9;color: #f00;">保存</button>'
    ].join('');
}
...
$('#image').change(function (e) {
    ...
    fileReader.onload = function (ev) {
        ...
        if (persons.length != 0) {
            if (!persons[0].no || !persons[0].userName || !persons[0].name || !persons[0].password|| !persons[0].answerPhone ) {
                layer.msg('数据模板不正确');
...
function designQuestionnaire() {
    ...
    if (ifDesignQuestionnaire == "false") {
        ...
        _$.cookie("questionnaireId", questionnaireId);
        let qId = _$.base64.encode(questionnaireId);
